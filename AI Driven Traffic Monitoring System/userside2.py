...
net = cv2.dnn.readNet("dnn_model/yolov4-416.weights", "dnn_model/yolov4-416.cfg")
...
def main_app():
    ...
    cam_location = tk.Label(app, text="CAM 22 Tabuan National Highway", width=20, padx=20, height=1,
                            fg="white", bg="#202124", font=title_font, anchor="w")
    cam_location.grid(row=1, column=0, columnspan=1, sticky="nsew")
    ...
    video_frame = tk.Frame(main_background, background="#202124", padx=10, pady=5)
    video_frame.grid(row=0, column=0, rowspan=4, columnspan=4, sticky="nsew")

    vid_widget = tk.Label(video_frame, width=video_width, height=video_height, background="#202124",
                          highlightbackground=border_color, highlightthickness=5)
    ...
    # Vehicle Model Panel
    labels_label = tk.Label(app, text="Vehicle Model Count", fg="white", bg="#202124", font=text_font, justify="center")
   ...
   # test_label.grid(row=1, column=4, sticky="n", padx=10, pady=30)
    ...
    def view_traffic_history():
        ...
        mycursor.execute("SELECT id, cam_location, date_time, daily_congestion, congestion_time FROM traffic_history WHERE cam_location = %s",(cam_location_value,))
    ...
    def view_traffic_weekly_history():
        ...
        mycursor.execute(
            "SELECT id, cam_location, start_date, end_date, date_time, weekly_congestion, total_congestion_time FROM traffic_history_weekly WHERE cam_location = %s",(cam_location_value,))
        data = mycursor.fetchall()
    ...
    def view_traffic_monthly_history():
        ...
        mycursor.execute(
            "SELECT id, cam_location, start_date, end_date, date_time, monthly_congestion, total_congestion_time FROM traffic_history_monthly WHERE cam_location = %s",(cam_location_value,))
        data = mycursor.fetchall()
    ...
    def detect_objects():
        ...
        while True:
            ...
            box_x, box_y, box_w, box_h = 700, 0, 250, 1000
